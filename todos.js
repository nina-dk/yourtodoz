...
app.post("/lists", requiresAuthentication,
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let newList = await store.createTodoList(todoListTitle);
      if (!newList) {
        req.flash("error", "The list title must be unique.");
        rerenderNewList();
      }
...
app.get("/lists/:todoListId",
  ...
  catchError(async (req, res, next) => {
    ...
    if (todoList === undefined) throw new Error("Not found.");
      ...
      let todoListInfo = {
        done: store.isDoneTodoList(todoList),
...
app.post("/lists/:todoListId/complete_all", 
  ...
  catchError(async (req, res) => {
    let store = res.locals.store;
    let todoListId = +req.params.todoListId;
    let completed = await store.markAllDone(todoListId);
...
app.post("/lists/:todoListId/todos",
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let addTodo = await store.addTodo(todoListId, todoTitle);
      if (!addTodo) throw new Error("Not found.");
