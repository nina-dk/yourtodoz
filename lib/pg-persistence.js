...
module.exports = class PgPersistence {
  ...
  // Returns a promise that resolves to `true` if a todo list with the specified title
  // exists in the list of todo lists, `false` otherwise.
  async existsTodoListTitle(title) {
    const FIND_TODO_LIST = "SELECT * FROM todolists " +
                            "WHERE title = $1 " +
                              "AND username = $2";
  ...
  // Changes the title of the todo list with the specified id. Returns a promise
  // that resolves to `true` on success, `false` on failure.
  async changeListTitle(todoListId, title) {
  ...
  // Deletes the todo list with the specified id. Returns a promise that
  ...
  // Marks all todos in the specified todo list as done. Returns a promise that
  // resolves to `true` on success, `false` if the todo list doesn't exist.
  async markAllDone(todoListId) {
    const MARK_DONE = "UPDATE todos SET done = TRUE " +
                       "WHERE todolist_id = $1"
                         "AND username = $2";
  ...
  // title. Returns a promise that resolves to `true` on success, `false`
  // on failure.
  async addTodo(todoListId, todoTitle) {
    const CREATE_TODO = "INSERT INTO todos (title, todolist_id, username) " + 
                             "VALUES ($1, $2, $3)";

    let result = await dbQuery(CREATE_TODO, todoTitle, todoListId, this.username);
    return result.rowCount > 0;
  }
  ...
  // Returns a promise that evaluates to `true` is a user with that username
  // and password exists, `false` otherwise.
  ...
  async sortedTodos(todoList) {
    ...
    let result = await dbQuery(SORT_TODOS, todoList.id, this.username);
    return result.rows;
  }
